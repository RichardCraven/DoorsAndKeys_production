...
function AutomatonConversionProgressBar({ converting, isVendorCell, is3x3Structure }) {
    const [now, setNow] = React.useState(Date.now());
    ...
    const barBorder = isPlayerClaim ? '#38bdf8' : '#ef4444';
    const barGlow = isPlayerClaim ? 'rgba(56, 189, 248, 0.9)' : 'rgba(239, 68, 68, 0.9)';
    const barGradient = isPlayerClaim
    ...
    return (
        <div style={{
            ...
            overflow: 'hidden',
            zIndex: 100,
            boxShadow: `0 0 10px ${barGlow}, 0 2px 6px rgba(0,0,0,0.9)`,
            pointerEvents: 'none',
            padding: '1px',
            ...
            <div style={{
                width: `${progress * 100}%`,
                height: '100%',
                backgroundColor: isPlayerClaim ? '#38bdf8' : '#ef4444',
...
function DestroyedCooldownBadgeOverlay({ contains, tileProps }) {
    const [now, setNow] = React.useState(Date.now());
    const destroyedAt = contains?.destroyedAt || tileProps?.destroyedAt;
    const disabledUntil = contains?.disabledUntil || tileProps?.disabledUntil;
    const duration = contains?.destroyedDuration || tileProps?.destroyedDuration || (disabledUntil && destroyedAt ? (disabledUntil - destroyedAt) : 10000);
    ...
    if (!destroyedAt) return null;
    const elapsed = Math.max(0, now - destroyedAt);
    if (elapsed >= duration || (disabledUntil && now >= disabledUntil)) {
        return null;
    }
...
function Tile(props) {
    ...
    const hasConvertingTarget = !!(
        props.converting ||
        props.convertingTarget ||
        (props.contains && (props.contains.convertingTarget || props.contains.convertingOutpost)) ||
        (currentTileForContains && (currentTileForContains.convertingTarget || (currentTileForContains.contains && (currentTileForContains.contains.convertingTarget || currentTileForContains.contains.convertingOutpost))))
    );
    ...
    const isVendorType = (val) => {
        ...
        if (typeof val === 'object' && val !== null) {
            ...
            if (sub.includes('domain_node') || sub.includes('dark_domain_node') || sub.includes('node') || sub.includes('earthen_fort') || sub.includes('outpost') || sub.includes('observer') || sub.includes('hut') || sub.includes('farm') || sub.includes('house') || sub.includes('locus')) {
                return false;
            }
        ...
        if (s.includes('domain_node') || s.includes('dark_domain_node') || s.includes('node') || s.includes('earthen_fort') || s.includes('outpost') || s.includes('observer') || s.includes('hut') || s.includes('farm') || s.includes('house') || s.includes('locus')) {
            return false;
        }
        const multiKeys = [
    ...
    const is2x2StructureSelf = !isPaletteTile && !isSingleTile && (
        ...
        thisKey.includes('sawmill') || thisKey.includes('lumber_mill') || thisKey.includes('ore_mine') ||
        thisKey.includes('slate_mine') || thisKey.includes('fungal_nursery') || thisKey.includes('mine') ||
        thisKey.includes('naked_trees_3') || thisKey.includes('naked_trees_4') || thisKey.includes('naked_mountains_2') ||
        isVendorType(thisKey) || !!containsObj?.isMultiTile || !!props.isMultiTile || !!containsObj?.isLarge || !!props.isLarge
    );
    ...
    const findNearbyStructureAnchor = () => {
        if (isPaletteTile || !boardTilesForContains || isDarkColor || color === 'black' || isSingleTile) return null;
        ...
        // Check if an anchor tile is at top-left, top, or left.
        // NOTE: bottom_right (-16) MUST be checked before left (-1), otherwise a bottom_right tile
        // would check its bottom_left neighbor at -1, mistake it for an anchor, and falsely report 'top_right'!
        const checks = [
        ...
        for (const { dRow, dCol, role, anchorOffset } of checks) {
            if (cRow >= dRow && cCol >= dCol) {
                ...
                if (aTile && aTile.color !== 'black' && (aTile.contains || aTile.building || aTile.image)) {
                    ...
                    if (aKey.includes('observer') || aKey.includes('outpost') || aKey.includes('earthen_fort') || aKey.includes('hut') || aKey.includes('domain_node') || aKey.includes('dark_domain_node') || aKey.includes('node') || aKey.includes('farm') || aKey.includes('house') || aKey.includes('locus')) continue;
                    
                    const aIs2x2 = (aKey.includes('domain_monolith') || aKey.includes('dark_domain_monolith') || (aKey.includes('monolith') && !aKey.includes('shrine')) || isVendorType(aKey));
                    if (aIs2x2) {
                        // If this tile has a known structure key, ensure anchor is compatible
                        if (thisKey && !aKey.includes(thisKey) && !thisKey.includes(aKey)) {
                            const isCompatible = (thisKey.includes('sawmill') && aKey.includes('lumber_mill')) ||
                                                 (thisKey.includes('lumber_mill') && aKey.includes('sawmill')) ||
                                                 (thisKey.includes('war_camp') && aKey.includes('war_fort')) ||
                                                 (thisKey.includes('war_fort') && aKey.includes('war_camp'));
    ...
    const isAnchorSingle = (() => {
        if (is2x2StructureSelf) return false;
        ...
        if (anchorId !== undefined && anchorId !== null && boardTilesForContains && boardTilesForContains[anchorId]) {
            ...
            return aKey.includes('observer') || aKey.includes('outpost') || aKey.includes('earthen_fort') || aKey.includes('hut') || aKey.includes('farm') || aKey.includes('house') || aKey.includes('domain_node') || aKey.includes('dark_domain_node') || aKey.includes('node') || aKey.includes('locus');
        }
        return false;
    })();
    ...
    const isAutomatonUnit = !!(
        ...
        (props.contains && typeof props.contains === 'object' && (props.contains.isAutomaton || props.contains.subtype === 'automaton')) ||
        ...
        >
           ...
           { (!isVendorCell || getVendorCellRole() === 'anchor') && (props.sabotageProgress !== undefined && props.sabotageProgress !== null) && (
            ...
            { (!isVendorCell || getVendorCellRole() === 'anchor') && (props.upgradeProgress !== undefined && props.upgradeProgress !== null) && (
                <div style={{
                    ...
                    <div style={{
                        width: `${Math.min(100, Math.max(0, props.upgradeProgress * 100))}%`,
                        height: '100%',
           ...
           { color !== 'black' && (props.isDisabledOutpost || (props.disabledUntil && Date.now() < props.disabledUntil)) && (
            ...
            { color !== 'black' && (() => {
                ...
                if (nearbyAnchorInfo && nearbyAnchorInfo.role && nearbyAnchorInfo.role !== 'anchor') return null;
                if (containsObj?.vendorCell && containsObj.vendorCell !== 'anchor') return null;
                if (props.vendorCell && props.vendorCell !== 'anchor') return null;
                ...
                const isSingleTileStructure = sKey.includes('earthen_fort') || sKey.includes('outpost') || sKey.includes('observer') || sKey.includes('domain_node') || sKey.includes('dark_domain_node') || sKey.includes('node') || sKey.includes('farm') || sKey.includes('house') || sKey.includes('locus');
                    ...
                    if (sb && Array.isArray(sb.miniboards) && typeof props.globalX === 'number' && typeof props.globalY === 'number') {
                        ...
                        for (let mbIdx = 0; mbIdx < 9; mbIdx++) {
                            ...
                            for (let tIdx = 0; tIdx < 225; tIdx++) {
                                ...
                                const isMonoDestroyed = !!(
                                    (c && (c.hp <= 0 || !!c.destroyedAt || (c.disabledUntil && Date.now() < c.disabledUntil))) ||
                                    (t.hp <= 0 || !!t.destroyedAt || (t.disabledUntil && Date.now() < t.disabledUntil)) ||
                                    (t.contains && typeof t.contains === 'object' && (t.contains.hp <= 0 || !!t.contains.destroyedAt || (t.contains.disabledUntil && Date.now() < t.contains.disabledUntil)))
                                );
                                if (isMonoDestroyed) continue;
                    ...
                    >
                     ...
                     { (props.territory || props.contains?.territory || currentContains?.territory || props.boardTiles?.[props.index]?.territory || props.boardTiles?.[props.id]?.territory) && (() => {
                         ...
                         if (clan.includes('cave')) {
                          ...
                          const isDomainMonolithTerritory = !isPygmyClan && (
                              isFriendly ||
                              clan.includes('hostile') ||
                              clan.includes('enemy') ||
                              !!props.insidePerfectSquareDomain ||
                              !!props.territoryMonolithId ||
                              !!props.contains?.territoryMonolithId ||
                              !!currentContains?.territoryMonolithId ||
                              !!props.boardTiles?.[props.index]?.territoryMonolithId ||
                              !!props.boardTiles?.[props.id]?.territoryMonolithId
                          );
                          return (
                              <div 
                                  ...
                                  style={{
                                      ...
                                      boxShadow: (isDomainMonolithTerritory || isFriendly) ? 'none' : `inset 0 0 5px ${borderColor}`, 
                                      border: (isDomainMonolithTerritory || isFriendly) ? 'none' : `1px dashed ${borderColor}`, 
                                      zIndex: 1, 
                                      pointerEvents: 'none', 
                      ...
                      {/* Intense light source glow emanating from behind monster/pygmy portrait */}
                      {isMonsterOrPygmyTile && (!isBlackTile || props.inSuperboard) && props.type !== 'overlay-tile' && ((color !== 'black' && currentTileColor !== 'black') || props.inSuperboard) && (() => {
                          ...
                          const isNeutralUnit = !isAlliedUnit && !!(
                              ...
                              (currentContains?.isPocketPygmy && !currentContains?.isHostile && currentContains?.faction !== 'hostile' && currentContains?.faction !== 'enemy')
                          );

                          // Vibrant, highly intense color-coded glows:
                          // Allied = Electric Cyan/Blue
                          const blueGradient = 'radial-gradient(circle at center, rgba(56, 189, 248, 1) 0%, rgba(14, 165, 233, 0.85) 38%, rgba(2, 132, 199, 0.5) 68%, transparent 95%)';
                          // Neutral = Warm Amber/Gold
                          const amberGradient = 'radial-gradient(circle at center, rgba(250, 204, 21, 1) 0%, rgba(245, 158, 11, 0.85) 38%, rgba(217, 119, 6, 0.5) 68%, transparent 95%)';
                          // Hostile = Crimson Red
           ...
           {!isLayeredForestTile && !isLayeredMountainTile && resolvedPortraitUrl && props.optionType !== 'delete' && props.optionType !== 'voidfill' && !(props.contains && (props.contains === 'shrine' || props.contains.type === 'shrine')) && !(props.data && props.data.type === 'soul_shard') && (() => {
                    ...
                    : (isEnlargeableStructure && isOccupied 
                        ...
                        : (isLocusActiveOrAdjacent
                            ...
                            : (isEncompassedByFriendlyDomain 
                                ? `scale(${1.5 * autoUnitScale}) ${rotationDeg ? `rotate(${rotationDeg}deg)` : ''}`.trim() 
                                : (isAutomatonUnit
                                    ? `scale(${autoUnitScale})`
                ...
                const isRiftEmbers = sKey.includes('rift_embers');

                if (isRiftEmbers && !isPaletteTile && vendorCellRole !== 'anchor') {
                    return null;
                }
           ...
           { (Boolean(props.data && props.data.dead) || (typeof props.hp === 'number' && props.hp <= 0) || (props.data && typeof props.data.hp === 'number' && props.data.hp <= 0)) && (
            ...
            {/* Automaton & Outpost / Building Conversion Progress Bar Overlay */}
            {(() => {
           ...
           {showDelayedHoverLabel && props.delayedHoverLabel && (
            ...
            {/* Destroyed Badge Overlay with 10s Cooldown Progress Bar */}
            {(((props.contains?.hp !== undefined && props.contains?.hp <= 0) || (props.hp !== undefined && props.hp <= 0)) && (props.contains?.destroyedAt || props.destroyedAt) && (!props.contains?.vendorCell || props.contains.vendorCell === 'anchor')) && (
                <DestroyedCooldownBadgeOverlay contains={props.contains} tileProps={props} />
            )}

            {/* Earthen Fort level badge (levels >= 1 show a number in the top right of the icon) */}
            {(() => {
                const bSubtype = containsSubtype || (containsObj && containsObj.subtype) || (containsObj && containsObj.building);
                const isFort = bSubtype === 'earthen_fort' || props.building === 'earthen_fort' || props.image === 'earthen_fort' || props.image === 'buildable_earthen_fort' || (props.generatorData && props.generatorData.key === 'earthen_fort') || (containsObj && containsObj.generatorData && containsObj.generatorData.key === 'earthen_fort');
                const lvl = (containsObj && containsObj.level) || 
                            (props.contains && props.contains.level) || 
                            (currentContains && currentContains.level) || 
                            (props.generatorData && props.generatorData.level) ||
                            (containsObj && containsObj.generatorData && containsObj.generatorData.level) ||
                            props.level || 1;
